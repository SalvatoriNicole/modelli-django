...
from phytophtora.models import Phytophtora
...
#@csrf_exempt
@api_view(['GET', 'POST'])
def phytophtora_list(request, format=None):
    if request.method == 'GET':
        return HttpResponse("I want POST requests")
    elif request.method == 'POST':
        #data = JSONParser().parse(request)
        serializer = PhytophtoraListSerializer(data=request.data, many=True)
        ...
        if serializer.is_valid():
             ...
             if bool(state[0])==True:
                 ...
                 T_min=float(state[0]['T_min'])
                 T_max=float(state[0]['T_max'])
                 T_opt=float(state[0]['T_opt']) 
                 W_min= float(state[0]['W_min'])
                 W_max=float(state[0]['W_max'])
             else:
             ...
             for j in range(0,len(data)):
                ...
                for days in range(index,len(data_daily)):
                   ...
                   # T_min=5
                   # T_max=30 #questi presi da Magarey
                    
                    ## questi tre presi da Abuley et al che però è per early blight e no late blight..
                   # T_opt=25 #20 questi altri erano di tomidis che però è di occhio di pavone
                   # W_min= 4 #12
                   # W_max=24
                    ...
                    #Tomodis et al., 2021 (che però è per occhio di pavone)
                    ...
                    while j <= len(T)-1:
                        ...
                        if infection1==1:
                            incubation=0.012*T[j]**2 - 0.636*T[j]+11.22
                        else:
                            incubation=0
                    ...
                    df_days={'doy':doy[days], 'Infection': infection1, 'hours_of_wetness_t0':hours_of_wetness_t0, 'hours_of_dry_t0':hours_of_dry_t0, 'incubation':incubation}
                    df.append(df_days)
...
def phytophtora_detail(request, pk):
    try:
        phytophtora= Phytophtora.objects.get(pk=pk)
    except Phytophtora.DoesNotExist:
        return HttpResponse(status=404)

    if request.method == 'GET':
        serializer = PhytophtoraListSerializer(phytophtora)
        return JsonResponse(serializer.data, safe=False)

    elif request.method == 'PUT':
        data = JSONParser().parse(request)
        serializer = PhytophtoraListSerializer(phytophtora, data=data)
        if serializer.is_valid():
            serializer.save()
